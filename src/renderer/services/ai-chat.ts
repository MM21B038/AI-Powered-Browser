...
export async function runAiChatPipeline(opts: {
  ...
  /** When true, do not inject saved skills or /skill mentions. */
...
async function buildSystemPromptForApi(
  ...
  let text = (baseOverride ?? systemPromptForWorkspace(scope)).trim();
  if (disableSkillInjection) return text;
